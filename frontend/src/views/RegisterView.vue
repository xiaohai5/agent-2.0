<template>
  <section class="ios-page auth-view ios-scroll">
    <PagePanel title="创建账户" description="填写基础信息后自动登录，后续可在个人资料中查看账户状态。" eyebrow="Register">
      <form class="form-stack" @submit.prevent="submitRegister">
        <IosField label="用户名">
          <input v-model="app.register.username" autocomplete="username" placeholder="设置用户名" />
        </IosField>

        <IosField label="邮箱">
          <input v-model="app.register.email" autocomplete="email" type="email" placeholder="填写邮箱地址" />
        </IosField>

        <IosField label="密码">
          <input v-model="app.register.password" autocomplete="new-password" type="password" placeholder="设置登录密码" />
        </IosField>

        <div class="ios-action-row">
          <IosButton class="grow" :disabled="app.loading.auth" type="submit">
            {{ app.loading.auth ? "提交中..." : "注册并登录" }}
          </IosButton>
          <IosButton variant="secondary" @click="router.push('/account')">返回登录</IosButton>
        </div>

        <div class="ios-status">{{ app.authStatus.value }}</div>
      </form>
    </PagePanel>
  </section>
</template>

<script setup>
import { useRouter } from "vue-router";
import IosButton from "../components/IosButton.vue";
import IosField from "../components/IosField.vue";
import PagePanel from "../components/PagePanel.vue";
import { useAssistantApp } from "../composables/useAssistantApp";

const app = useAssistantApp();
const router = useRouter();

async function submitRegister() {
  await app.doRegister();
  if (app.state.token) {
    router.push({ name: "chat" });
  }
}
</script>

<style scoped>
.auth-view {
  display: grid;
  align-items: center;
}

.form-stack {
  display: grid;
  gap: 14px;
}

.grow {
  flex: 1;
}
</style>
